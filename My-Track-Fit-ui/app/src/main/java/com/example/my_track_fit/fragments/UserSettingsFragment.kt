...
import com.example.my_track_fit.LoginActivity
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnUploadFiles.setOnClickListener {
            Thread {
                try {
                    ...
                    val url = java.net.URL("$BASE_URL/api/upload-user-files")
                    val conn = url.openConnection() as java.net.HttpURLConnection
                    conn.requestMethod = "POST"
                    conn.setRequestProperty("Content-Type", "application/json")
                    conn.doOutput = true
        ...
        btnDownloadFiles.setOnClickListener {
            Thread {
                try {
                    val email = userEmail ?: ""
                    val url = java.net.URL("$BASE_URL/api/download-user-files?email=$email")
                    val conn = url.openConnection() as java.net.HttpURLConnection
